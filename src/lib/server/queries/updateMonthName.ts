...
export async function updateMonthName ({ year, month }: input):Promise<returnResult|undefined> {
	/*
	verify year.id exists
	verify month.id exists
	check if month.name being requested is a valid month name
	check if this year already has the month.name being requested

	if all checks out then update the month with the month.id to the new name
	*/
