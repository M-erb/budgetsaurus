...
export async function saveYearMonthDb({
	year: yearName,
	month: monthName
}: {
	year: number
	...
	if (!yearFromDb) {
		const { res: resNewYear, err: errNewYear } = await to(
			db
				.insert(years)
				.values({ name: String(yearName), note: '' })
				.returning()
		)
	...
	const { res: monthFromDb, err: errMonthFromDb } = await to(
		db.query.months.findFirst({
			where: (months, { eq, and }) =>
				and(eq(months.name, monthName), eq(months.yearId, result.year!.id!))
		})
	)
	...
	if (!monthFromDb) {
		const { res: resNewMonth, err: errNewMonth } = await to(
			db.insert(months).values({ name: monthName, yearId: result.year!.id! }).returning()
		)
